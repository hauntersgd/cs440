...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model, max_seq_length):
        ...
        for pos in range(max_seq_length):
            idx_tensor_2i = torch.arange(0, d_model, 2)
            idx_tensor_2i_1 = torch.add(idx_tensor_2i, 1)
            for j in range(idx_tensor_2i.shape[0]):
                pe[pos, idx_tensor_2i[j]] = math.sin(pos * math.exp(idx_tensor_2i[j] * (-math.log(10000)/d_model)))
                pe[pos, idx_tensor_2i_1[j]] = math.cos(pos * math.exp(idx_tensor_2i[j] * (-math.log(10000)/d_model)))
