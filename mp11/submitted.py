...
def get_returns(rollout_buffer: utils.RolloutBuffer, discount_factor=0.95):
    ...
    """
    # YOUR CODE HERE
    returns = torch.zeros(len(rollout_buffer.rewards), 1, dtype=torch.float32)

    terminal_idx = [-1]
    for i in range(len(rollout_buffer.terminateds)):
        if rollout_buffer.terminateds[i]:
            terminal_idx.append(i)

    for k in range(len(terminal_idx) - 1):
        for i in range(terminal_idx[k] + 1, terminal_idx[k + 1] + 1):
            for j in range(i, terminal_idx[k + 1] + 1):
                returns[i] += (discount_factor ** (j - i)) * rollout_buffer.rewards[j]
...
def get_advantages(value_net: nn.Module,
    ...
    with torch.no_grad():
        ...
        advantage = advantage / (torch.std(advantage) + EPSILON)
...
def get_value_net_loss(value_net: nn.Module,
    ...
    return loss(returns, value_net(observation))
    # raise NotImplementedError()
...
def get_vanilla_policy_gradient_loss(policy: nn.Module,
    ...
    """
    # YOUR CODE HERE
    num_rows, num_cols = policy(observation).shape
    vpgl = torch.zeros(1, dtype=torch.float32)

    for t in range(num_rows):
        vpgl += policy(observation)[t][action[t]] * return_or_advantage[t]

    vpgl = -vpgl/len(return_or_advantage)
        ...
        for _ in range(training_epochs_per_rollout):
            ...
            for batch_start in range(0, rollout_buffer.final_size, minibatch_size):
                ...
                policy_gradient_kwargs = dict(
                    ...
                    return_or_advantage=    advantages[batch_start:batch_stop] if get_advantages is not None else get_returns(rollout_buffer), # Fill in
...
def get_PPO_policy_gradient_loss (policy: nn.Module,
    ...
    for t in range(num_rows):
        ...
        r_t = (torch.exp(policy(observation)[t, action[t]]))/(torch.exp(old_logits[t , action[t]]))
        r_t_clipped = torch.clamp(r_t, 1 - ppo_clip, 1 + ppo_clip)
        vpgl += min(r_t * return_or_advantage[t], r_t_clipped * return_or_advantage[t])
