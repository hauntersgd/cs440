for sentence in test:
          ...
          for w in range(1, len(sentence)):
               for t in range(len(tag_set)):
                    max_prob = float('-inf')
                    for r in range(len(tag_set)):
                         if sentence[w] in word_set:
                              transition_prob = trellis[r][w - 1] + transition[r][t]
          ...
          # for t in range(len(tag_set) - 1):
          #      for w in range(1, len(sentence)):
          #           if sentence[w] in word_set:
          #                trellis[t][w] = observation[sentence[w]][tag_lookup[t]]
          #           else:
          #                trellis[t][w] = observation["OOV"][tag_lookup[t]]
